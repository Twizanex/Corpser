...
if (elgg_instanceof($page_owner, 'user')) {
	$url_segment = 'story/archive/' . $page_owner->username;
} else {
	$url_segment = 'story/group/' . $page_owner->getGUID() . '/archive';
}

// This is a limitation of the URL schema.
if ($page_owner && $vars['page'] != 'friends') {
	$dates = array_reverse(get_entity_dates('object', 'story', $page_owner->getGUID()));
	
	if ($dates) {
		$title = elgg_echo('story:archives');
		$content = '<ul class="story-archives">';
