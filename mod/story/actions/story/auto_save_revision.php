...
if ($title && $description) {
	...
	if ($guid) {
		...
		if (elgg_instanceof($entity, 'object', 'story') && $entity->canEdit()) {
			...
			$error = elgg_echo('story:error:post_not_found');
		}
	} else {
		$story = new ElggBlog();
		$story->subtype = 'story';
		...
		$story->status = 'unsaved_draft';
		$story->access_id = ACCESS_PRIVATE;
		$story->title = $title;
		$story->description = $description;
		$story->excerpt = elgg_get_excerpt($excerpt);
		...
		if (!$story->save()) {
			$error = elgg_echo('story:error:cannot_save');
		}
	}
	...
	if (!$error) {
		...
		if ($auto_save_annotations = $story->getAnnotations('story_auto_save', 1)) {
		...
		if (!$auto_save) {
			$annotation_id = $story->annotate('story_auto_save', $description);
		...
		if (!$annotation_id) {
			$error = elgg_echo('story:error:cannot_auto_save');
		}
	}
} else {
	$error = elgg_echo('story:error:missing:description');
}
...
if ($error) {
	...
	$msg = elgg_echo('story:message:saved');
	$json = array('success' => TRUE, 'message' => $msg, 'guid' => $story->getGUID());
	echo json_encode($json);
}
exit;
